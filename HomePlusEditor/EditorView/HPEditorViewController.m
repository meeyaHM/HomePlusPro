...
- (void)viewDidLoad
{
    ...
    [self.view addGestureRecognizer:[[HPManager sharedInstance] _rtGestureRecognizer]];
}
...
- (HPEditorViewNavigationTabBar *)defaultTabBar
{
    ...
    if (![[[HPEditorManager sharedInstance] editingLocation] isEqualToString:@"SBIconLocationFolder"])
    {
...
{
    if ([[[HPEditorManager sharedInstance] editingLocation] isEqualToString:@"SBIconLocationDock"])
    {
        return;
    }
    AudioServicesPlaySystemSound(1519);
    [[HPEditorManager sharedInstance] setEditingLocation:@"SBIconLocationDock"];
...
- (HPControllerView *)settingsView
{
    ...
    if (!_settingsView)
    {
        ...
        imageView.image = [HPEditorManager sharedInstance].dynamicallyGeneratedSettingsHeaderImage;
