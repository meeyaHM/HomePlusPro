...
// TODO: all of this is bad, just need to get around to burning it
...
- (void)layoutSubviews
{
    ...
    self.iconLabelAlpha = ((([[[HPDataManager sharedInstance] currentConfiguration] integerForKey:@"HPDataIconLabels"]?:0) == 0) ? 1 : 0);
}
...
{
    return (([[[HPDataManager sharedInstance] currentConfiguration] integerForKey:@"HPDataIconLabels"]?:0) == 0) ? %orig : 0;
}

- (void)setIconLabelAlpha:(CGFloat)alpha
{
    %orig((([[[HPDataManager sharedInstance] currentConfiguration] integerForKey:@"HPDataIconLabels"]?:0) == 0) ? alpha : 0);
}
